...
export function TypeSelectionButton({ selection_type, disable }: TypeSelectionButton) {
  ...
  return (
    <button
      ...
      <div
        className={`flex aspect-square w-32 h-32 sm:h-44 sm:w-44 -translate-y-3 items-center justify-center rounded-full transition ${
          !disable && 'group-hover:-translate-y-4 group-active:-translate-y-1'
        } md:h-64 md:w-64 ${button_style.layer2}`}>
        <div className="flex aspect-square w-24 h-24 sm:h-36 sm:w-36 overflow-hidden rounded-full bg-slate-300 md:h-52 md:w-52">
          <div
            className={`flex aspect-square w-24 h-24 sm:h-36 sm:w-36 translate-y-2 items-center justify-center rounded-full bg-white transition ${
              !disable && 'group-hover:translate-y-1 group-active:translate-y-3'
            }  md:h-52 md:w-52`}>
            <button_style.icon className="" classGroup="md:h-32 md:w-32 sm:h-24 sm:w-24 w-12 h-12" />
          </div>
        </div>
      </div>
    </button>
